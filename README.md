...
Dobby AI does not collect accounts, cookies, advertising identifiers, prompts, page content, or API keys. It sends one daily anonymous usage event with aggregate request and outcome counters split between free-proxy and own-key mode, a random installation ID, and the extension version. See [PRIVACY.md](PRIVACY.md) for details.
